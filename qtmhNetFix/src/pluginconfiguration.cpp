...
void PluginConfiguration::load(){
	...
	disablePowerSaving = helper->load("disablePowerSaving", false).toBool();
	wlanCheck = helper->load("wlanCheck", true).toBool();
}
...
void PluginConfiguration::save(){
	...
	helper->save("disablePowerSaving", disablePowerSaving);
	helper->save("wlanCheck", wlanCheck);
}
